	...
	while(choice){
		...
		switch(choice){
			...
			case 4: cout<<"Shear >> ";
					...
					cout<<m.at<float>(1,0)<<"  "<<m.at<float>(0,0)<<" ";
					cout<<n.at<float>(1,0)<<"  "<<n.at<float>(0,0)<<endl;
	 ...
	 break; break;
			...
			case 10:cout<<"Reconstruction >> ";
					...
					cout<<path<<x1<<y1<<x2<<y2<<x3<<y3<<x4<<y4<<x1_<<y1_<<x2_<<y2_<<x3_<<y3_<<x4_<<y4_<<endl;
					 // Read the g_image file
	 				image = imread(path);
					 // Check for failure
					 if (image.empty())
